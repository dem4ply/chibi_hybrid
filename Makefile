...
lint: ## check style with flake8
	flake8 chibi_hybrid tests

test-all: ## run tests on every Python version with tox
	tox

coverage: ## check code coverage quickly with the default Python
	coverage run --source chibi_hybrid setup.py test
	coverage report -m
	coverage html
	$(BROWSER) coverage_html_report/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/chibi_hybrid.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ chibi_hybrid
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
pep8:
	...
	@pep8 --statistics chibi_hybrid tests

flakes:
	@echo "Running flakes tests..."
	@pyflakes chibi_hybrid tests
